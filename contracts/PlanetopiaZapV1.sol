...
import {IPlanetopiaPair} from "./interfaces/IPlanetopiaPair.sol";
import {IPlanetopiaRouter02} from "./interfaces/IPlanetopiaRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by PlanetopiaSwap teams.
 */
contract PlanetopiaZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped FON (WFON)
    IWETH public WFON;

    // PlanetopiaRouter interface
    IPlanetopiaRouter02 public planetopiaRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from PlanetopiaSwap)
    uint256 public constant MINIMUM_AMOUNT = 1000;

    // Maximum reverse zap ratio (100 --> 1%, 1000 --> 0.1%)
    uint256 public maxZapReverseRatio;

    // Address PlanetopiaRouter
    address private planetopiaRouterAddress;

    // Address Wrapped FON (WFON)
    address private WFONAddress;
    ...
    // tokenToZap = 0x00 address if FON
    event ZapIn(
    ...
    // token0ToZap = 0x00 address if FON
    ...
    // tokenToReceive = 0x00 address if FON
    event ZapOut(
        address indexed lpToken,
    ...
    /*
     * @notice Fallback for WFON
     */
    receive() external payable {
        assert(msg.sender == WFONAddress);
    }

    /*
     * @notice Constructor
     * @param _WFONAddress: address of the WFON contract
     * @param _planetopiaRouter: address of the PlanetopiaRouter
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WFONAddress,
        address _planetopiaRouter,
        uint256 _maxZapReverseRatio
    ) {
        WFONAddress = _WFONAddress;
        WFON = IWETH(_WFONAddress);
        planetopiaRouterAddress = _planetopiaRouter;
        planetopiaRouter = IPlanetopiaRouter02(_planetopiaRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap FON in a WFON pool (e.g. WFON/token)
     * @param _lpToken: LP token address (e.g. PLANET/FON)
     * @param _tokenAmountOutMin: minimum token amount (e.g. PLANET) to receive in the intermediary swap (e.g. FON --> PLANET)
     */
    function zapInFON(address _lpToken, uint256 _tokenAmountOutMin) external payable nonReentrant {
        WFON.deposit{value: msg.value}();

        // Call zap function
        uint256 lpTokenAmountTransferred = _zapIn(WFONAddress, msg.value, _lpToken, _tokenAmountOutMin);
     ...
     * @param _lpToken: LP token address (e.g. PLANET/BUSD)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. PLANET) in the intermediary swap (e.g. BUSD --> PLANET)
     */
    function zapInToken(
        address _tokenToZap,
    ...
    /*
     * @notice Zap 1 token and FON, rebalance them to 50-50, before adding them to LP
     * @param _token1ToZap: address of token1 to zap
     * @param _token1AmountIn: amount of token1 to zap
    ...
    /*
     * @notice Zap a LP token out to receive FON
     * @param _lpToken: LP token address (e.g. PLANET/WFON)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in FON/WFON) in the intermediary swap (e.g. PLANET --> FON)
     */
    function zapOutFON(
        address _lpToken,
        ...
        // Call zapOut
        uint256 tokenAmountToTransfer = _zapOut(_lpToken, WFONAddress, _tokenAmountOutMin);

        // Unwrap FON
        WFON.withdraw(tokenAmountToTransfer);

        // Transfer FON to the msg.sender
        (bool success, ) = msg.sender.call{value: tokenAmountToTransfer}(new bytes(0));
        require(success, "FON: transfer fail");
     ...
     * @param _lpToken: LP token address (e.g. PLANET/BUSD)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. PLANET or BUSD)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. PLANET) in the intermediary swap (e.g. BUSD --> PLANET)
     */
     ...
     * @dev Use WFON for _tokenToZap (if FON is the input)
    ...
    {
        address token0 = IPlanetopiaPair(_lpToken).token0();
        address token1 = IPlanetopiaPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IPlanetopiaPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = planetopiaRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = planetopiaRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
     ...
     * @dev Use WFON for _token0ToZap (if FON is the input)
    ...
    {
        require(
            _token0ToZap == IPlanetopiaPair(_lpToken).token0() || _token0ToZap == IPlanetopiaPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IPlanetopiaPair(_lpToken).token0() || _token1ToZap == IPlanetopiaPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IPlanetopiaPair(_lpToken).token0()) {
    ...
    /*
     * @notice View the details for single zap
     * @dev Use WFON for _tokenToReceive (if FON is the asset to be received)
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IPlanetopiaPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IPlanetopiaPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = planetopiaRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = planetopiaRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IPlanetopiaPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "PlanetopiaRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "PlanetopiaRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        planetopiaRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), planetopiaRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(planetopiaRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = planetopiaRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = planetopiaRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = planetopiaRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = planetopiaRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = planetopiaRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
